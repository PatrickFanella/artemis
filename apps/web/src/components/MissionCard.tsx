...
export function MissionCard({ mission }: { mission: Mission }) {
  ...
  return (
    <Link
      ...
      className="block bg-space-dark border border-space-gray/50 rounded-xl p-6 hover:border-artemis-blue/50 transition-colors group"
    >
      <div className="flex items-start justify-between mb-3">
        <h3 className="text-xl font-semibold group-hover:text-artemis-blue transition-colors">
          {mission.name}
        </h3>
        <StatusBadge status={mission.status} />
      </div>
      <p className="text-lunar-white/60 text-sm mb-4">{mission.tagline}</p>
