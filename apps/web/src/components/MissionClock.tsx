...
export function MissionClockDisplay({ clock }: { clock: MissionClock }) {
  ...
  return (
    <div className="bg-space-dark border border-space-gray/50 rounded-xl p-4 md:p-6">
      {/* Live indicator */}
      ...
      <div className="text-center mb-4">
        ...
        <div className="met-display text-4xl md:text-5xl font-bold text-artemis-gold">
          ...
          <span className="text-artemis-gold/40">:</span>
        ...
        <div className="flex items-center justify-center gap-1 mt-1 text-xs text-lunar-white/30">
          <span>DD</span>
          <span>:</span>
          <span>HH</span>
          <span>:</span>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-space-black/50 rounded-lg p-3 text-center">
          <p className="text-2xl font-bold text-artemis-blue">
            {`FD${pad(flightDay)}`}
          </p>
          <p className="text-xs text-lunar-white/40 mt-1">Flight Day</p>
          <p className="text-xs text-lunar-white/30 mt-0.5">
            +{pad(fdHours)}:{pad(fdMins)} into FD
          </p>
        </div>
        <div className="bg-space-black/50 rounded-lg p-3 text-center">
          <p className="text-2xl font-bold text-artemis-cyan">
            {Math.min(Math.round((met.totalSeconds / 718200) * 100), 100)}%
          </p>
          <p className="text-xs text-lunar-white/40 mt-1">Mission Progress</p>
          <div className="mt-2 h-1.5 bg-space-gray rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-artemis-blue to-artemis-cyan rounded-full transition-all duration-1000"
              style={{
